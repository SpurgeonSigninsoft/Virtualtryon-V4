...
async function removeBackground(file){
  ...
  try{
    ...
    const blob=await removeBackground(file,{progress:(key,current,total)=>{if(key.includes('compute')) $('#glassesFileName').textContent=`Processing ${Math.round(current/total*100)}%`}});
...
async function fallbackBackgroundRemoval(src){
  const img=await loadImage(src),c=el.work,x=c.getContext('2d',{willReadFrequently:true});c.width=img.naturalWidth;c.height=img.naturalHeight;x.drawImage(img,0,0);const d=x.getImageData(0,0,c.width,c.height),p=d.data;
...
async function setGlasses(file){if(!file)return;try{const processed=await removeBackground(file);state.glasses=await loadImage(processed);el.glassesPreview.src=processed;$('#glassesFileName').textContent=file.name;$('#glassesDrop').classList.add('hidden');$('#glassesPreviewCard').classList.remove('hidden');$('#toStep2').disabled=false;toast('Frame is ready.')}catch(e){console.error(e);toast('That image could not be processed. Try another file.')}}
